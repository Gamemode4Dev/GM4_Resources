...
def beet_default(ctx: Context):
    ctx.require(copy_folders)


@configurable(validator=CopyFilesOptions)
def copy_folders(ctx: Context, opts: CopyFilesOptions):
        ...
        for dst, src in files.items():
            if len(src) == 1:
                ...
                for path, file_type in src:
                    pack.extra[f"{dst}/{path.relative_to(ctx.directory.joinpath(ctx.assets.name,dst))}"] = file_type(source_path=path)
...
def resolve_file_mapping(
    ...
    return {
        key: [
            ...
            for filename in directory.rglob(pattern)
        ]
